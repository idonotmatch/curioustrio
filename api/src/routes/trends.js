...
router.get('/scenario-memory/watching', async (req, res, next) => {
  try {
    const user = await getUser(req);
    if (!user) return res.status(401).json({ error: 'User not synced' });
    ...
    try {
      await refreshConsideringScenarios(user, {
        limit: Math.max(1, Math.min(Number(req.query.limit) || 10, 20)),
      });
    } catch (refreshErr) {
      console.error('[scenario memory] watched refresh failed (non-fatal):', refreshErr.message);
    }
