...
export default function FeedScreen() {
  ...
  useFocusEffect(useCallback(() => {
    // softRefreshMine picks up any expenses written to cache by confirm/approve
    // (prependToExpenseMonthCaches sets ts=0, so TTL check always fires a background fetch)
    // without hammering the server when the user tabs in and out during a normal session.
    softRefreshMine();
